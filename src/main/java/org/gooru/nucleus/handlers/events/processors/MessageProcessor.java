...
class MessageProcessor implements Processor {
  ...
  private static final Logger TRANSMIT_FAIL_LOGGER =
      LoggerFactory.getLogger("org.gooru.nucleus.transmission-errors");
  ...
  public JsonObject process() {
    ...
    try {
      ...
      TRANSMIT_FAIL_LOGGER.error(ResponseFactory
          .generateErrorResponse((JsonObject) (message != null ? message.body() : null))
          .toString());
    }
    return result;
  }
  ...
  private JsonObject processEventAssessmentQuestionTagAggregate() {
    try {
      ...
      if (result != null) {
        ...
        return ResponseFactory.generateResponse(request, result,
            MessageConstants.EST_ITEM_TAG_AGGREGATE);
      }
    } catch (Throwable t) {
  ...
  private JsonObject processEventUserPasswordChange() {
    try {
      ...
      if (result != null) {
        LOGGER.debug("result returned: {}", result);
        return ResponseFactory.generateResponse(request, result,
            MessageConstants.EST_USER_PASSWORD_CHANGE);
      }
  ...
  private JsonObject processEventUserPasswordReset() {
    try {
      ...
      if (result != null) {
        LOGGER.debug("result returned: {}", result);
        return ResponseFactory.generateResponse(request, result,
            MessageConstants.EST_USER_PASSWORD_RESET);
      }
  ...
  private JsonObject processEventUserPasswordResetTrigger() {
    try {
      ...
      if (result != null) {
        LOGGER.debug("result returned: {}", result);
        return ResponseFactory.generateResponse(request, result,
            MessageConstants.EST_USER_PASSWORD_RESET_TRG);
      }
  ...
  private JsonObject processEventCollectionCollaboratorUpdate() {
    try {
      ProcessorContext context = createContext();
      JsonObject result =
          RepoBuilder.buildCollectionRepo(context).updateCollectionCollaboratorEvent();
      if (result != null) {
        LOGGER.debug("result returned: {}", result);
        return ResponseFactory.generateItemCollaboratorUpdateResponse(request, result);
      }
