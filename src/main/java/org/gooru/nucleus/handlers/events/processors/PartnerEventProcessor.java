...
public class PartnerEventProcessor implements Processor {
	...
	private void processContentCreationEvent() {
		...
		MessageDispatcher.getInstance().sendMessage2Kafka(EventResponseConstants.EVENT_ITEM_CREATE, courseEvent);
		...
		units.forEach(unitId -> {
			...
			lessons.forEach(lessonId -> {
				...
				JsonObject lessonResult = RepoBuilder
						.buildLessonRepo(createContext(MessageConstants.MSG_OP_EVT_LESSON_CREATE, lessonEventBody))
				...
				List<AJEntityCollection> collections = RepoBuilder.buildCollectionRepo(null)
						.fetchCollectionsByCUL(courseId, unitId, lessonId);
				...
				collections.forEach(collection -> {
					...
					JsonObject collectionEventBody = new JsonObject().put(EventRequestConstants.ID, collectionId);
					...
					if (format.equalsIgnoreCase(AJEntityCollection.FORMAT_EX_COLLECTION)) {
						...
						collectionResult = RepoBuilder
								.buildCollectionRepo(createContext(eventName, collectionEventBody))
					...
					JsonObject collectionRequest = new JsonObject().put(EventRequestConstants.EVENT_NAME, eventName)
							.put(EventRequestConstants.SESSION_TOKEN,
									request.getString(EventRequestConstants.SESSION_TOKEN))
	...
	private ProcessorContext createContext(String eventName, JsonObject eventBody) {
