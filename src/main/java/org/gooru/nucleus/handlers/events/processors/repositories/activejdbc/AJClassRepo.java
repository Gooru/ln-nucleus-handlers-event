...
public class AJClassRepo implements ClassRepo {
  ...
  public JsonObject classContentVisibleEvent() {
    /*
     * Base.open(DataSourceRegistry.getInstance().getDefaultDataSource()); String classId =
     * context.eventBody().getString(EventRequestConstants.ID); LazyList<AJEntityClass> classes =
     * AJEntityClass.where(AJEntityClass.SELECT_CONTENT_VISIBILITY, classId); if (classes.isEmpty())
     * { LOGGER.warn( "Not able to find class '{}'", classId); return null; } Base.close(); return
     * new JsonObject(classes.get(0).getString(AJEntityClass.CONTENT_VISIBILITY) );
     */
    return new JsonObject();
  }
