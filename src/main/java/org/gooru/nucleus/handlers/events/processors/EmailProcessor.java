...
public class EmailProcessor implements Processor {
  ...
  public JsonObject process() {
    ...
    emailData.stream().forEach(data -> {
      HttpClientRequest emailRequest =
          httpClient.getEmailHttpClient().post(httpClient.emailEndpoint(), responseHandler -> {
            if (responseHandler.statusCode() == HttpConstants.HttpStatus.SUCCESS.getCode()) {
              LOGGER.info("email sent to '{}' for event: {}", data, eventName);
            } else {
              LOGGER.warn("email not sent for event {}, HttpStatusCode: {}, requestPayload: {}",
                  eventName, responseHandler.statusCode(), data.toString());
            }
          });
  ...
  private JsonArray processEmailForResetPassword() {
    return new EmailDataBuilder().setEmailTemplate(EmailConstants.TEMPLATE_RESET_PASSWORD)
        .setResultData(result).setEventData(message).build();
  }

  private JsonArray processEmailForResetPasswordTrigger() {
    return new EmailDataBuilder().setEmailTemplate(EmailConstants.TEMPLATE_RESET_PASSWORD_TRG)
        .setResultData(result).setEventData(message).build();
  }
  ...
  private JsonArray processEmailForResourceDelete() {
    return new EmailDataBuilder().setEmailTemplate(EmailConstants.TEMPLATE_RESOURCE_DELETE)
        .setResultData(result).build();
  }

  private JsonArray processEmailForCollectionCollaboratorUpdate() {
    return new EmailDataBuilder()
        .setEmailTemplate(EmailConstants.TEMPLATE_COLLECTION_COLLABORATOR_INVITE)
        .setResultData(result).setEventData(message).build();
  }

  private JsonArray processEmailForCourseCollaboratorUpdate() {
    return new EmailDataBuilder()
        .setEmailTemplate(EmailConstants.TEMPLATE_COURSE_COLLABORATOR_INVITE).setResultData(result)
  ...
  private JsonArray processEmailToInviteStudent() {
    ...
    return new EmailDataBuilder().setEmailTemplate(EmailConstants.TEMPLATE_USER_INVITE_CLASS)
        .setResultData(result).build();
  }

  private JsonArray processEmailToFollowProfile() {
    return new EmailDataBuilder().setEmailTemplate(EmailConstants.TEMPLATE_PROFILE_FOLLOW)
        .setResultData(result).build();
  }
  ...
  private boolean checkWelcomeEmailSent() {
    ...
    return data != null && !data.isEmpty()
        ? data.getBoolean(AJEntityUserState.WELCOME_EMAIL_SENT_KEY, true)
        : true;
  }
}
