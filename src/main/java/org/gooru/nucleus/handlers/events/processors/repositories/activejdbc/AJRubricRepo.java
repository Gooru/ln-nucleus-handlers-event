...
public class AJRubricRepo implements RubricRepo {
    ...
    private JsonObject getRubric(String rubricId) {
        ...
        LazyList<AJEntityRubric> rubrics = AJEntityRubric.findBySQL(AJEntityRubric.FETCH_RUBRIC , rubricId);
        if (!rubrics.isEmpty()) {
            result = new JsonObject(new JsonFormatterBuilder()
                .buildSimpleJsonFormatter(false, AJEntityRubric.FETCH_RUBRIC_FIELDS).toJson(rubrics.get(0)));
        }
